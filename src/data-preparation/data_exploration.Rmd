...
First, the \textit{numeric} variables we considered as \textit{apartment-related} and from whihc we will build the correlogram were the following: 
```{r, include=FALSE}
...
my_colors <- c("darkblue", "#FFFFFF", "darkred")
...
```{r}
numeric_df <- clean_data %>% dplyr::select_if(is.numeric)
...
```{r}
clean_data <- clean_data %>% dplyr::select(-availability_90, -review_scores_accuracy, -review_scores_rating,
