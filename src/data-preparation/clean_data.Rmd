...
```{r, echo=FALSE}
raw_data <- read.csv("../../gen/data-preparation/input/merged_data.csv", fileEncoding = "UTF-8")
```
...
\section{Manage Amenities Variable}
...
selected_cols <- head(raw_data, 5) %>%
  dplyr::select("Air_conditioning":"Bathtub")
...
\section{Manage Missing Values}
After completing all the intended data cleaning operations, the removal of observations containing missing values (i.e., NAs) is warranted.
...
By inspecting the plot, it is possible to notice that: 

\begin{itemize}
  \item Numeric Variables:
    \begin{itemize}
      \item The majority of variables with missing values are in 'numeric' format.
      \item Most of these belong to the "review-related" variables.
      \item Other numeric variables containing missing values include 'bathroom,' 'bedrooms,' 'beds,' and more.
    \end{itemize}
  \item Date Variables:
    \begin{itemize}
      \item Three variables are in 'date' format: 'last\_review,' 'first\_review,' and 'host\_since.'
    \end{itemize}
  \item Categorical Variables:
    \begin{itemize}
      \item Two variables are in 'factor' format: 'host\_acceptance\_rate\_percentage' and 'host\_response\_time\_percentage.'
    \end{itemize}
\end{itemize} 
...
\section{Managing Factor Variables}
