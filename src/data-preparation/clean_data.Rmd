...
```{r, echo=FALSE}
data <- read.csv("../../gen/data-preparation/input/data.csv", fileEncoding = "UTF-8")
```
...
```{r}
cols_to_remove <- sapply(data, function(x) all(is.na(x)))
...
Therefore, as done before for the texttt{property\_type} variable, in order to reduce the computational time required by our regression model to predict listings prices, we decided to remove the \texttt{neighbourhood\_cleansed} levels with less than 4 observations. 
