...
```{r, echo=FALSE}
# Load the R object
raw_data <- readRDS("../../gen/data-preparation/input/merged_data.rds")
```
...
\begin{enumerate}
    ...
    \item[\Roman{enumi}II.] Management of Observations with Missing Values: Ultimate elimination of all observations containing missing values (NAs) across any variables within the dataset, acknowledging the incompatibility of regression models and correlations with missing data.
    \item[\Roman{enumi}III.] Management of Price Outliers: Subsequent removal of observations with outliers in the \texttt{price} variable, as it serves as the dependent variable (DV) in our regression model.
    \item[\Roman{enumi}IV.] Management of Unnecessary Variables: Elimination of variables that lack relevance to our regression model, such as \texttt{listing\_url}.
...
\textbf{Missing Values per Variable Dataframe:} The subsequent code was employed to generate a two-column dataframe called 'missing_data'. Its first column contains the variable names from the 'raw_data' dataset, while the second column displays the number of missing values for each variable.
...
Below is an excerpt from the previously generated dataframe 'missing_data', displaying the first five rows. This dataset will be utilized in the subsequent section for building the plot "Number of Missing Values per Variable".
```{r, include=FALSE}
t5 <- head(missing_data, 5) %>%
  gt() %>%
  tab_style(
    style = cell_text(weight = "bold"),
    locations = cells_column_labels(columns = everything())
...
p5 <- ggplot(data = missing_data, aes(x = variable, y = missing_values, fill = missing_values)) +
  ...
  scale_fill_gradient(low = "#FEFBBD", high = "darkgreen") +
...
By inspecting the plot, it is possible to notice that the majority of variables with missing values are in 'numeric' format. Most of these belong to the "review-related" variables. Other numeric variables containing missing values include 'bedrooms,' 'beds,' 'review\_per\_month', and more. 
...
\begin{enumerate}
  \item[\Roman{enumi}I.] \textbf{Data Pre-Procesing:} Data preprocessing is necessary to convert the 'amenities' column into a format suitable for analysis.
    \begin{enumerate}
...
\end{enumerate}
  \item[\Roman{enumi}II.] \textbf{Amenity Frequency Analysis:} Filtering frequently occurring amenities focuses the analysis on the most relevant data, preventing an overwhelming number of binary columns.
    \begin{enumerate}
      ...
      \item[\alph{enumii}] Filtering Common Amenities: Frequency-Based Filtering: To optimize the utility of the generated binary columns, a thoughtful filtering process was executed. Specifically, amenities appearing at least 10,000 times were selected for binary column creation. This selection criterion was chosen to prioritize commonly offered amenities while simplifying the dataset's structure and avoiding the creation of an excessively large number of binary columns. Before filtering, the number of newly created columns was over 13,000, making the dataset not suitable for further analysis. We also tried to include amenities that appeared in less than 10 listings, but even then the number of new columns created would have been more than 11,000. 
    \end{enumerate}
  \item[\Roman{enumi}III.] \textbf{Amenity Binary Column Creation:} The 'check\_amenity' function streamlines the process of determining amenity presence, by providing clear indicators of amenity presence or absence in each listing.
    \begin{enumerate}
      \item[\alph{enumii}] Amenity Presence Check Function: A custom function, 'check\_amenity,' is defined. This function checks whether a specific amenity is present in the 'amenities' column of each listing. It returns "YES" if the amenity is found and "NO" if it's not.
      \item[\alph{enumii}] Creating Binary Columns: For each amenity that met the filtering criteria, a new binary column is created. The column name is derived from the amenity name with spaces, colons, and hyphens removed and replaced by underscores.
    \end{enumerate}
\end{enumerate}
...
# Calculate the frequency of each unique amenity
amenity_counts <- table(all_words)

# Filter amenities that appear at least 10000 times
selected_amenities <- names(amenity_counts[amenity_counts >= 10000])

# Create a function to check if an amenity is present
check_amenity <- function(amenities, amenity_name) {
  as.factor(ifelse(str_detect(amenities, paste0('(?i)', amenity_name)), "YES", "NO"))
}

# Create a new column for each unique amenity with spaces, colons, and hyphens removed and replaced by underscores
for (amenity in selected_amenities) {
  col_name <- gsub("[:\\- ]", "_", amenity, perl = TRUE)  # Remove colons, hyphens, and spaces and replace with underscores
  raw_data <- raw_data %>%
    mutate(!!col_name := check_amenity(amenities, amenity))
}
```


```{r, echo=FALSE}
rm(word_lists, all_words, amenity, amenity_counts, col_name, selected_amenities, check_amenity)
```
...
The initial procedural step involves the application of the \texttt{droplevels} function to factor variables within the dataset. As missing value-containing observations have been previously removed, there is a possibility that certain factor variables may now contain vacant levels, which should be eliminated as well. The subsequent code demonstrates this process, initially identifying all factor variables within the dataset, and subsequently, applying the \texttt{droplevels} function to each of them by using the \texttt{lapply} function.
...
As indicated in the table below, the variable \texttt{property\_type} exhibits some levels with a notably low frequency of observations.
(We have selected and displayed only 10 random levels as an example, recognizing that the \texttt{property\_type} variable comprises more than 25 levels with fewer than four observations each.)
```{r, include=FALSE}
...
In a manner similar to the previous step, the table below illustrates that the \texttt{neighbourhood\_cleansed} variable exhibits several levels with a notably low frequency of observations. (We have selected and displayed only 10 random levels as an example, recognizing that the \texttt{neighbourhood\_cleansed} variable comprises more than 250 levels with fewer than four observations each.)
```{r}
grouped_data <- raw_data %>% 
...
In conclusion, the cleaned dataset is preserved for future purposes. It is stored both as a CSV file for convenient inspection and as an RDS file, retaining all the data manipulation steps, including variable format conversions.
```{r}
# Save the data as a CSV file
write.csv(raw_data, file = "../../gen/data-preparation/input/clean_data.csv", fileEncoding = "UTF-8", row.names = FALSE)
